...
var BODY_SEPARATORS = /* @__PURE__ */ new Set([CANONICAL_BODY_SEPARATOR, LEGACY_BODY_SEPARATOR]);
...
function parseReviewMark(markdown) {
  const diagnostics = [];
...
function parseCommentInner(inner, raw, startLine, endLine) {
  const diagnostics = [];
...
function readType(meta, diagnostics, line) {
  if (meta.type) {
...
function readStatus(value, diagnostics, line) {
  if (!value) {
    return "open";
  }
...
function collectLeadingMetaLines(lines) {
  const metaLines = [];
  for (const line of lines) {
...
function looksLikeTableContinuation(lines, index) {
  return lines[index].includes("|");
}
function findTargetBlock(blocks, commentLine) {
...
function attachReviews(blocks, comments) {
  ...
  return blocks.filter((block) => reviews.has(block.index)).map((block) => ({
    block,
    comments: reviews.get(block.index) ?? []
  }));
}
function findDuplicateIds(comments) {
...
function countReviewOpenings(markdown) {
...
function excerpt(markdown, max) {
  return markdown.replace(/```[\s\S]*?```/g, "[code]").replace(/[#>*_`[\]()!-]/g, "").replace(/\s+/g, " ").trim().slice(0, max);
}
  ...
  <div class="reviewmark-shell">
    <header class="reviewmark-header">
      <div>
        ...
        <p>${doc.comments.length} comment${doc.comments.length === 1 ? "" : "s"} \xB7 ${doc.diagnostics.length} diagnostic${doc.diagnostics.length === 1 ? "" : "s"}</p>
      </div>
      <nav>
...
function renderArticle(doc, commentsByBlock) {
...
function renderInlineComment(comment) {
  ...
  <div class="reviewmark-comment-body">${g.parse(comment.body, { async: false })}</div>
</aside>`;
}
function renderSidebar(comments) {
  const items = comments.map((comment) => {
    const targetLink = comment.attachedToBlockIndex !== void 0 ? `#block-${comment.attachedToBlockIndex + 1}` : `#${comment.id}`;
    return `<li>
  <a href="${targetLink}">
    <strong>${escapeHtml(comment.id)}</strong>
    <span>${escapeHtml(comment.metadata.author)} \xB7 ${escapeHtml(comment.metadata.type)} \xB7 ${escapeHtml(comment.metadata.status)}</span>
  </a>
</li>`;
  }).join("");
